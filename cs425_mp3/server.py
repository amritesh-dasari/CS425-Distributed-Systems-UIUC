...
SDFS_path = "/home/shikhar4/cs425_mp3/sdfs/"
class FileSystem:
	def __init__(self):
...
class Server:
	...
	def run(self):
		...
		# broadcaster_thread = threading.Thread(target=self.broadcaster)
		fd_thread.start()
		cmdblk_thread.start()
	...
	def mbslistupdateChecker(self,newmembershiplist):
		flag = False
		newmembershiplistkeys = list(newmembershiplist.keys())
		for node in newmembershiplistkeys:
			if node not in self.tempmbslist:
				self.tempmbslist[node] = newmembershiplist[node]['status']
				# flag=True	
		for node in newmembershiplistkeys:
			if self.tempmbslist[node] != newmembershiplist[node]['status']:
				flag = True
				self.tempmbslist[node] = newmembershiplist[node]['status']
		return flag
	...
	def broadcaster(self):
		...
		while True:
			if self.primary == self.host:
				if (self.mbslistupdateChecker(membershiplist)):
					log = "Changes in nodes status detected, sending fileSystem Broadcast..."
					print(log)
					...
					# multicast this put operation to all active members of membership list?
						tempcopy = list(membershiplist.keys())
						for host in tempcopy:
							if membershiplist[host]["status"] == "ACTIVE" and host!=self.host:
								log = f"Sending SDFS BROADCAST to {host}"
								
								connection.sendto(json.dumps(packet).encode('utf-8'), (host, 8100))
				...
				try:
					...
					if packet:
						...
						if packet_type == "PUT":
							...
							log = f"[INFO]: Recieved SDFS BROADCAST message from {sender_host}"
							
							# print(log)
							self.sdfs.fileMapper = data.get('vmMapper')
							self.sdfs.vmMapper = data.get('fileMapper')
