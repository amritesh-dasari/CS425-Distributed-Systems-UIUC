...
class Server:
    ...
    def update_neighbours(self):
        current_neighbours=self.neighbours
        membershiplist = self.mbslist.membershiplist
        if len(current_neighbours)<4:
            for node in membershiplist.keys():
                if (node!=self.host and membershiplist[node]['status']=="ACTIVE" and node not in current_neighbours):
                    current_neighbours.append(node)
                if len(current_neighbours)==4:
                    distance=[]
                    for neighbour in current_neighbours:
                        neighbournum = int(neighbour.split("-")[2][2:4])
                        if (neighbour in membershiplist.keys() and membershiplist[neighbour]['status']=="ACTIVE"):
                            distance.append(min(abs(nodenum-neighbournum),10-abs(nodenum-neighbournum)))
                        else:
                            distance.append(100)
                    self.distance = distance
                    break
        else:
            nodenum = int(self.host.split("-")[2][2:4])
            distance=[]
            for neighbour in current_neighbours:
                neighbournum = int(neighbour.split("-")[2][2:4])
                if (neighbour in membershiplist.keys() and membershiplist[neighbour]['status']=="ACTIVE"):
            ...
            if self.distance!=distance:
                for node in membershiplist.keys():
                    if (node!=self.host and membershiplist[node]['status']=="ACTIVE" and node not in current_neighbours):
                        ...
                        tempdistance = min(abs(nodenum-tempnodenum), 10-abs((nodenum-tempnodenum)))
                        ...
                        if tempdistance>currentmaxdistance:
                            ...
                            for index in range(len(distance)):
                                if tempdistance<distance[index]:
                                    del current_neighbours[index]
                                    current_neighbours.append(node)
                                    break
        self.neighbours = current_neighbours
    ...
    def print_membership_list(self):
        ...
        for key in membershiplist.keys():
            print(f"{key}: [id: {membershiplist[key]['id']}, timestamp: {membershiplist[key]['timestamp']}, status: {membershiplist[key]['status']}]")
